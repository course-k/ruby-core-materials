...
tasks/<課題番号>/
  ...
  original/<name>.rb           手本の原本（学習対象の定義だけ。学習者が写すのはこれだけ）
...
- 断片の連結・スクリプト化
- 原典の呼び出し例・`# =>` 注記・出力例を照合テスト（`copy_test/`）の assert へ写す（手本には呼び出しも表示も付加しない）
- 散文どおりの API 呼び出しの補完
...
「定義を 1 つ削った写し」「定義の中の文言を 1 箇所打ち間違えた写し」「セミコロン・4 スペース・`camelCase`
