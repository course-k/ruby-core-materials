...
開くファイル: `~/lab/ruby-learning/ruby-core-materials/tasks/rb1-09-blocks-procs/original/blocks_and_procs_test.rb`
...
1. `bin/check rb1-09-blocks-procs ../ruby-core` の全項目が `[合格]`（終了コード 0）。
   内訳は「写しの実行」「写しのアサーション数」「写しの書式」「`why.md` が雛形と差分あり」「確認課題テスト」「出力予測 01」の 6 項目。
