...
**課題ごとの表は手で書かない**——`bin/check` が `.check/` から状態と日付を書き直している。
手で足しても次に `bin/check` を走らせたときに消える。開いて `rb1-01-tooling` の行が
`完了` になっていることだけ確かめる。
...
`.check/rb1-01-tooling.yml` は `bin/check` が書く機械の記録で、表の正本。
自分では編集しない。
