...
- 教材の作成者から封緘の鍵ファイルを受け取っている（手順 3 で置く）。
...
### 手順 3 — 教材リポで `bundle install` し、封緘の鍵を置く

教材リポは `~/lab/ruby-learning/ruby-core-materials` にある前提で進む（前提の節）。

まず封緘の鍵を置く。教材の作成者から受け取った鍵ファイル（`seal.key`）を、次の場所へ置く。
**鍵は自分で作らない**——封緘は教材を作ったときの鍵で掛かっているので、
自分で作った鍵では開かない。
...
着手できなくなる**ので、受け取ったファイルは別の場所にも控えておく。
