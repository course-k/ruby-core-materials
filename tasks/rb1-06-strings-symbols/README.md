# rb1-06-strings-symbols — 文字列と Symbol

## この課題を終えると何ができるようになるか / 全体のどこにいるか

**位置**: Ruby 軸・レベル「基礎」・課題 6（全 21 課題の 6 本目）。

**到達点**: この課題を終えると、次のことができるようになる。

- 二重引用符と単一引用符の違い、式展開 `#{}`、隣り合うリテラルの連結、`%w` を使い分けられる。
- `# frozen_string_literal: true` が何をするマジックコメントかを説明でき、
  凍った文字列を書き換えようとしたときに何が起きるかを言える。
- `upcase` / `strip` / `split` / `gsub` / `start_with?` / `<<` / `upcase!` など主要な String メソッドを使える。
- Symbol と String の役割の違い（同一性か内容か）を説明でき、`to_s` / `to_sym` で行き来できる。
- `String#encoding` が UTF-8 を既定とすること、`force_encoding` が解釈だけを変え `encode` が中身を変えることを説明できる。
...
- 課題 5（`tasks/rb1-05-control-flow-io/README.md`）までが完了していること。
...
開くファイル: `~/lab/ruby-learning/ruby-core-materials/tasks/rb1-06-strings-symbols/original/strings_and_symbols.rb`

この手本は、公式リファレンスの Literals・Comments（frozen_string_literal Directive 節）・
String・Symbol・Encodings の各節の例をつないだもの。
手本は**定義だけ**。定義を呼び出して結果を確かめるコードは、教材が配る照合テスト
`copy_test/strings_and_symbols_test.rb` にある。**写さない。読んでよい。**
...
エディタで `rb1-06-strings-symbols/strings_and_symbols.rb` を新規作成し、手本を手で打ち込む。
...
bundle exec rubocop --config .rubocop.yml ../ruby-core/rb1-06-strings-symbols/strings_and_symbols.rb
```
...
教材リポの `tasks/rb1-06-strings-symbols/why.md` を、成果リポの `rb1-06-strings-symbols/why.md` へコピーする。
...
書き終えたら、教材リポの `tasks/rb1-06-strings-symbols/commentary.md` を開いて読む。
...
> `/why-review rb1-06-strings-symbols`
...
走らせると、教材リポの claim 表（`tasks/rb1-06-strings-symbols/claims.yml`）と `why.md` を突き合わせ、
...
結果は `~/lab/ruby-learning/docs/grading/rb1-06-strings-symbols.md` に残る（成果リポには置かない）。

### 手順 6 — 確認課題

文字列を整える `Normalizer` を、判定テストが通るように実装する。
...
判定テストは教材リポの `tasks/rb1-06-strings-symbols/exercise/test/normalizer_test.rb` にある。**読んでよい。**

**作るもの**: 次の 2 つのメソッドを持つモジュール。

- `Normalizer.call(text)` — 前後の空白を落とし、内側の連続した空白を 1 つの空白にまとめ、小文字にした
  **新しい文字列**を返す。渡された文字列そのものは変えない。
- `Normalizer.key(text)` — `call` と同じ整え方をしたうえで、空白を `_` に置き換えた Symbol を返す。

（この `Normalizer` は、この先の課題で作る道具でも使う形——入力を揃えてから比べる——の最小形である。）
...
1. `bin/check rb1-06-strings-symbols ../ruby-core` の全項目が `[合格]`（終了コード 0）。
...
記録先は成果リポの `progress.md`。課題ごとの記録の表に `rb1-06-strings-symbols` の行を足し、
...
次に開くファイル: `~/lab/ruby-learning/ruby-core-materials/tasks/rb1-07-collections-enumerable/README.md`
