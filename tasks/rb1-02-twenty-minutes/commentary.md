...
1. **irb のやりとりを実行できるファイルの形に直した。** `irb(main):001:0>` のような
   プロンプト表示と `=> 5` のような戻り値表示を落とし、代わりに minitest の
   `assert_equal 5, 3 + 2` の形で「何が返るか」を書いた。
2. **minitest の骨格を足した。** `require "minitest/autorun"` と
   `class TwentyMinutesTest < Minitest::Test` と `def test_…` は教材が足したもので、
   原典には無い。
...
4. **`if __FILE__ == $0` の本体をテストメソッドへ移した。** 原典のスクリプトは
   末尾に `if __FILE__ == $0` で囲まれた実行部分を持つ。この書き方（ファイルを直接実行したときだけ
   動かす）は課題 19 で扱うので、ここでは実行部分を `test_mega_greeter_…` の中へ移し、
   `puts` の出力を `assert_output` で確かめる形にした。`#!/usr/bin/env ruby` の行も同じ理由で落とした。
5. **書式を教材の RuboCop 設定に合わせた。** `3 ** 2` は `3**2` に詰めた
   （`**` の前後に空白を置かないのが RuboCop の既定）。ほかに空白・引用符の変更はない。
