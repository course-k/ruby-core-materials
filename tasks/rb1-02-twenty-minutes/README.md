...
- 自分で打った定義（メソッド・クラス）を、教材が用意したテストから呼び出して動くことを確かめられる。
...
この課題で初めて出る語: **照合テスト**は、教材が用意した、写しを読み込み、写しが定義したメソッドやクラスを呼び出して結果を確かめるテスト。
教材リポの `tasks/rb1-02-twenty-minutes/copy_test/` にある。**写さない。読んでよい。** 落ちたときは
「どのメソッドの・どの呼び出しが・何を出すべきか」が Expected（期待した値）／Actual（実際の値）で出る。
...
1 つのファイルにまとめたもの。手本にあるのは定義（`hi` / `Greeter` / `Greeter` の再オープン / `MegaGreeter`）だけ。
原典が irb で試している式や、`MegaGreeter` を動かす部分は、照合テスト `copy_test/twenty_minutes_test.rb`
の中にある。照合テストも開いてよい。自分の定義がどう呼ばれ、何を出すべきかが原典の部ごとに書いてある。
...
の行を消す。定義まで打ち終えていれば、追加で打つものは無い。手順 3 へ進み、照合テストと RuboCop が
指す箇所を直す。
...
何も表示されずに終われば読み込めている（手本は定義だけなので、実行しても出力は無い）。
`syntax error` などが出たら、表示された行番号の箇所を直す。
...
最終行に `8 runs, 27 assertions, 0 failures, 0 errors, 0 skips` が出れば写しは正しい。
`failures` や `errors` が 0 でないときは、表示される
「期待した値（Expected）／実際の値（Actual）」か、`undefined method 'xxx'`（メソッド名の打ち間違い）を
読んで直す。テスト名の `part1`〜`part3` が原典の部に対応する。
...
```
/why-review rb1-02-twenty-minutes
```

教材リポの claim 表（`tasks/rb1-02-twenty-minutes/claims.yml`）と `why.md` を突き合わせ、
claim ごとに一致度（一致 / 部分 / ずれ / 未言及）と到達時点（自力 / 突き合わせ後 / 未）を返す。
続けて §3 の質問に答える。答えには一次情報の URL と原文の引用が必ず付くので、
納得できないところは原典を自分で確かめられる。

**合否には効かない。** `bin/check` の 4 項目とは別で、走らせなくても課題は完了する。
結果は `~/lab/ruby-learning/docs/grading/rb1-02-twenty-minutes.md` に残る（成果リポには置かない）。
